	...
	class WC_SADAD_Gateway extends WC_Payment_Gateway {
		...
		public function process_payment( $order_id ) {
	        ...
	        $data = array(
                ...
                'msisdn'	    => $order_billing_phone,
                'email'        => $order_billing_email,
                'customer-name'	=> $order_billing_first_name . " " . $order_billing_last_name,
                'amount'	=>      strval($total),
                'description'	=> $order_id,
                'date'	=> $date->format('Y-m-d H:i:s'),
                'notification-mode'	=> '200',
                'success-url'   =>  site_url().'/wc-api/sadad_payment_success/'.$orderIdString,
                'error-url' =>  site_url().'/wc-api/sadad_payment_failure/'.$orderIdString,

            );
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, ($this->testmode ? "https://eps-net-uat.sadadbh.com" : "https://eps-net.sadadbh.com") . "/api/v2/web-ven-sdd/epayment/create/");
